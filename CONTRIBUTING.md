...
- **Apache Parquet** - company/job data stored in Parquet format, loaded client-side via WASM

### Rules

**Data loading is build-time for CSV sources, client-side for Parquet.** CSV data (H1B, VC, Private Markets) is parsed at build time. Parquet data (companies, jobs) is loaded client-side via WASM — keep payloads reasonable.

**No new server routes.** The site is fully static. Cloudflare Functions in `functions/` are for D1-backed features only.
