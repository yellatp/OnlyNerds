 ...
 */
/** Get the Google dork query for a target level label. */
export function getLevelQuery(levelLabel: string): string {
  return TARGET_LEVELS.find((l) => l.label === levelLabel)?.query ?? '';
}
